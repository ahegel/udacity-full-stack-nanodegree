...
if __name__ == '__main__':
    ...
    with open('output.txt', 'w') as f:
        f.write('1. What are the most popular {} articles of all time?\n'.format(n_articles))
        for article in top_3_articles:
            f.write(article[0] + ', ' + str(article[1]) + ' views\n')
        f.write('\n2. Who are the most popular article authors of all time?\n')
        for author in top_authors:
            f.write(author[0] + ', ' + str(author[1]) + ' views\n')
        f.write('\n3. On which days did more than {}% of requests lead to errors?\n'.format(error_perc))
        for error in error_days:
            clean_date = datetime.strptime(error[0], '%Y-%m-%d')
            f.write(clean_date.strftime('%B %-d, %Y') + ', ' + str(error[1]) + '% errors')
